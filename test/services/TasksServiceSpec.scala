...
class TasksServiceSpec extends BaseSpec {
  ...
  "invoking .get" - {
    ...
    "must return a Task from the repository if there is one for the given internal id and utr" in {
      ...
      val task = Task(trustees = true, settlors = false, protectors = false, beneficiaries = false, other = false, nonEeaCompany = false)
    ...
    "must return a Task from the repository if there is not one for the given internal id and utr" in {
      val task = Task(trustees = false, settlors = false, protectors = false, beneficiaries = false, other = false, nonEeaCompany = false)
  ...
  "invoking .set" - {
    ...
    "must set an updated Task" in {
      ...
      val task = Task(trustees = true, settlors = true, protectors = false, beneficiaries = false, other = false, nonEeaCompany = false)
